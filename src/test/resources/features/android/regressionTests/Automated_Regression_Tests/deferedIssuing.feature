...
# https://github.com/eu-digital-identity-wallet/eudi-doc-testing-application/issues/217
  ...
  @US_DI_TC_01
  Scenario Outline: Completing the deferred credential delivery process with Python Issuer
    Given the user is accessing <issuer> service
    When the user chooses to deliver a deferred credential to the wallet with <issuer>
    Then the EUDI Wallet application is opened
    And the issuance information is displayed to the user for <issuer>
    Then the user selects the ISSUE button for <issuer>
    And the user is redirected to the issuer service for authentication and authorization for <issuer>
    When the user completes authentication and confirms the issuance for <issuer>
    ...
    Then the issued credential information is displayed for <issuer>
    And the user presses the X button to close the document
    Then the document details are no longer displayed
    And the document appears on the documents screen for <issuer>
