@ANDROID @US_WRRPMAATCRM @Q2_2026
Feature: Wallet Interprets Credential Reuse Policy Metadata
  ...
  Scenario: User sees a normal credential-issuance flow
    Given the issuer metadata includes a credential_reuse_policy with at least one supported option
  ...
  Scenario: User is not asked to choose a reuse behavior
    Given the issuer metadata includes a credential_reuse_policy with multiple supported options
    When I proceed through credential issuance
  ...
  Scenario: Wallet automatically applies the issuer's preferred behavior
    Given the issuer metadata includes a credential_reuse_policy with a single option that the wallet supports
  ...
  Scenario: Wallet silently selects the first fully supported policy when multiple options exist
    Given the issuer metadata includes a credential_reuse_policy listing multiple options in order of preference
    When the wallet processes the issuer metadata
  ...
  Scenario: Issuance is blocked when the metadata lacks a mandatory baseline policy
    Given the issuer metadata's credential_reuse_policy does not include once_only or limited_time
    When the wallet processes the issuer metadata
  ...
  Scenario: Issuance is blocked when no reuse rules are provided
    Given the issuer metadata does not include a credential_reuse_policy
    When the wallet processes the issuer metadata
    Then I see an error and the issuance flow is blocked

  @US_WRRPMAATCRM_TC_07
  Scenario: Issuance proceeds when only the mandatory baseline policy is present
    Given the issuer metadata's credential_reuse_policy includes only once_only
    When the wallet processes the issuer metadata
    Then the wallet silently applies the once_only reuse method and issuance proceeds normally

  @US_WRRPMAATCRM_TC_08
  Scenario: Issuance proceeds when a non-mandatory policy is combined with a mandatory one
    Given the issuer metadata's credential_reuse_policy includes rotating-batch and limited_time
    When the wallet processes the issuer metadata
